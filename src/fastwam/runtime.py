...
def create_dream_fastwam(
    ...
    from .models.wan22.dream_fastwam import DreamFastWAM
    ...
    if isinstance(dream_query_config, DictConfig):
        dream_query_config = OmegaConf.to_container(dream_query_config, resolve=True)
    if dream_query_config is None:
        raise ValueError("`dream_query_config` is required for DreamFastWAM.")
    if not isinstance(dream_query_config, dict):
        raise ValueError(f"`dream_query_config` must resolve to a dict, got {type(dream_query_config)}")
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for DreamFastWAM.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
    ...
    return DreamFastWAM.from_wan22_pretrained(
        ...
        video_dit_config=video_dit_config,
        action_dit_config=action_dit_config,
        dream_query_config=dream_query_config,
        action_dit_pretrained_path=action_dit_pretrained_path,
        skip_dit_load_from_pretrain=bool(skip_dit_load_from_pretrain),
        mot_checkpoint_mixed_attn=bool(mot_checkpoint_mixed_attn),
        video_train_shift=float(video_scheduler.get("train_shift", 5.0)),
        ...
        loss_lambda_video=float(loss.get("lambda_video", 1.0)),
        loss_lambda_action=float(loss.get("lambda_action", 1.0)),
        loss_lambda_dream=float(loss.get("lambda_dream", 0.1)),
        loss_lambda_dyn=float(loss.get("lambda_dyn", 1.0)),
